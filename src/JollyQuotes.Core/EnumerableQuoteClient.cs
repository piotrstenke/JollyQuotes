...
namespace JollyQuotes
{
	...
	public abstract partial class EnumerableQuoteClient<T> : EnumerableQuoteResolver<T> where T : class, IQuote
	{
		...
		protected EnumerableQuoteClient(string source, bool includeBaseAddress = true) : base(Internals.CreateResolver(source, includeBaseAddress), source)
		{
		}
		...
		protected EnumerableQuoteClient(HttpClient client) : base(new HttpResolver(client), client.RetrieveSourceFromClient())
		{
		}
		...
		protected EnumerableQuoteClient(HttpResolver resolver) : base(resolver, resolver.BaseClient.RetrieveSourceFromClient())
		{
		}
